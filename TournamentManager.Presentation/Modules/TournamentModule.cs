...
namespace TournamentManager.Presentation.Modules
{
    public static class TournamentModule
    {
        public static void AddTournamentsEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            {
                var command = new CreateTournamentCommand(createTournamentRequest.Name, createTournamentRequest.Description);
            ...
            {
                var command = new UpdateTournamentCommand(id, updateTournamentRequest.Name, updateTournamentRequest.Description);
                var result = await mediator.Send(command, ct);
                return Results.Ok(result);
            }).WithTags("Tournaments").RequireAuthorization();
